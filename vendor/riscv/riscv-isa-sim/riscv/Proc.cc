...
namespace openhw
{
    st_rvfi Processor::step(size_t n)
    {
        ...
        rvfi.insn = (uint32_t) (this->get_state()->last_inst_fetched & 0xffffffffULL);
        ...
        rvfi.trap = this->which_trap;

#ifdef DPIDEBUG
        std::cerr << "[CC][TANDEM][PROC:" + to_string(this->id) + "[PC:" << std::hex << rvfi.pc_rdata << "][INS:" << rvfi.insn << "][RD:" << rvfi.rd1_addr << "][RDV:" << rvfi.rd1_wdata <<  std::endl;
#endif
      ...
      :  processor_t::processor_t(isa, cfg, sim, id, halt_on_reset, log_file, sout_)
    {
        ...
        std::cout << "[SPIKE-TANDEM] Configure core " << this->id << " with isa " << isa_str << std::endl;
